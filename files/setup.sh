...
echo "Installing proot-distro (1/1 command are processed right now)"

# proot-distro
pkg install proot-distro -y > /dev/null 2>&1

echo "Installing Debian (1/2 command are processed right now)"

# debian installation
proot-distro install debian > /dev/null 2>&1

echo "Copying XashDS folder (1/3 command are processed right now)"
